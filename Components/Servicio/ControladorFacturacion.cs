...
namespace Facturar.Components.Servicio
{
    public class ControladorFacturacion
    {
        ...
        public async Task CargarEstadoBorradorAsync()
        {
            ...
            if (DateTime.TryParse(fechaGuardada, out DateTime fecha)) FechaFacturaEnBorrador = fecha;
            else FechaFacturaEnBorrador = DateTime.Today;
        ...
        public async Task GuardarCambiosDraftItemAsync()
        {
            if (DraftItem.Identificador == 0)
            {
                ...
                var itemExistente = itemsActuales.FirstOrDefault(i => i.Producto.Equals(DraftItem.Producto, StringComparison.OrdinalIgnoreCase));
            ...
            else await _servicioFactura.ActualizarItem(DraftItem);
        ...
        {
            DraftItem = new FacturaItem { Identificador = item.Identificador, Producto = item.Producto, Cantidad = item.Cantidad, PrecioUnitario = item.PrecioUnitario };
        }
        ...
        public async Task<List<FacturaItem>> ObtenerItems() => await _servicioFactura.ObtenerItems();
        public async Task EliminarItem(int identificador) => await _servicioFactura.EliminarItem(identificador);
        ...
        public async Task GuardarFacturaActualAsync(DateTime fechaFactura, string nombreUsuario, List<FacturaItem> itemsDraft)
        {
            ...
            if (!itemsDraft.Any()) throw new Exception("No se puede guardar una factura vacía.");
            await _servicioFactura.GuardarFacturaCompletaAsync(fechaFactura, nombreUsuario, itemsDraft);
            await LimpiarEstadoBorradorAsync();
        }
        ...
        public async Task<List<Factura>> ObtenerFacturasGuardadasAsync() => await _servicioFactura.ObtenerFacturasGuardadasAsync();
        ...
        public async Task<ReporteAnual> GenerarReporteAnualAsync(int anio, string nombreUsuario)
        {
            ...
            var facturasPorMes = facturas.GroupBy(f => f.FechaCreacion.Month).ToDictionary(g => g.Key, g => g.ToList());
            for (int i = 1; i <= 12; i++)
            {
                var nombreMes = CultureInfo.CurrentCulture.DateTimeFormat.GetMonthName(i);
                var reporteMes = new ReporteMes { NumeroMes = i, NombreMes = nombreMes.Substring(0, 1).ToUpper() + nombreMes.Substring(1) };
                if (facturasPorMes.ContainsKey(i)) reporteMes.Facturas = facturasPorMes[i];
                reporte.Meses.Add(reporteMes);
            }
            return reporte;
        }
        public async Task<DashboardEstadisticas> ObtenerDatosDashboardAsync() => await _servicioFactura.ObtenerEstadisticasDashboardAsync();

        public async Task ArchivarFacturaAsync(int facturaID)
        {
            await _servicioFactura.CambiarEstadoArchivoAsync(facturaID, true);
        }
        public async Task DesarchivarFacturaAsync(int facturaID)
        {
            await _servicioFactura.CambiarEstadoArchivoAsync(facturaID, false);
        }
        public async Task<List<Factura>> ObtenerFacturasArchivadasAsync()
        {
            return await _servicioFactura.ObtenerFacturasArchivadasAsync();
        }
    }
}